...
const Instalations = () => {
 ...
 useEffect(()=>{
        ...
        const installapps=data?.filter(app=>storeAppsConvert.includes(app.id))
   ...
   setInstalls(update)
    ...
    return (
      ...
      <div className="flex flex-row justify-between mb-14">
        <h1 className="text-2xl btn ml-20">Apps Found {installs.length} </h1>
        <details className="dropdown mr-24">
          <summary className="btn m-1 w-52">Sort By {sort?sort:""}</summary>
          <ul className="menu w-52 ml-1 dropdown-content bg-base-100 rounded-box z-1 p-2 shadow-sm">
            <li >
              <a onClick={()=>sortHandleButton("High to Low")} className="btn mt-2 w-full">High to Low</a>
            </li>
            <li >
              <a onClick={()=>sortHandleButton("Low to High")} className="btn mt-2 w-full">Low to High</a>
            </li>
          </ul>
        </details>
      </div>
